...
const BestTimeTimeline = ({ pid }) => {
  ...
  return (
    <div className="p-6">
      {loading && <p className="text-center text-blue-500">Loading destination data...</p>}
        ...
        {destination.map((values, index) => (
          ...
          >
            {index !== destination.length - 1 && (
              <div className="hidden md:block absolute top-1/2 right-0 transform translate-x-full h-1 w-6 bg-blue-500"></div>
            )}
            <h3 className="text-lg font-bold text-blue-700">{values.period}</h3>
